...
class MemberUpdateEvent extends Event {
  public async run(
    ...
    if (oldMember.pending && !newMember.pending) {
      ...
      const welcomeChannel = newMember.guild.channels.resolve(
        env.WELCOME_CHANNEL
      ) as TextChannel;
      const logsChannel = newMember.guild.channels.resolve(
        env.LOGS_CHANNEL
      ) as TextChannel;
      ...
      interactiveVerifyURL.searchParams.append(
        'ch',
        (client.channels.resolve(env.VERIF_CHANNEL) as TextChannel).name
      );
      ...
      newMember.roles
        ...
        .then(() => {
          welcomeChannel
            .send(
              `¡Bienvenid@ <@!${newMember.user.id}> a la **Confederación de Fandom Hispano**!`,
              {
                embed: {
                  color: 2936518,
                  thumbnail: {
                  ...
                  fields: [
                    {
                      ...
                      value:
                        'Reacciona con el emoji 🌐 para ingresar como invitado.'
                    },
                    {
                  ...
                  if (reaction && reaction.emoji.name === '🌐') {
                    newMember.roles
                      ...
                      .then(() => {
                        ...
                        logsChannel
                          .send(
                            `✅ Se verificó a <@!${newMember.user.id}> como invitado`
                          )
                          .catch(client.logException);
                      })
                      .catch((err) => {
        ...
        .catch((err) => {
          logsChannel
            .send(
              `❌ Imposible agregar el rol Nuevo Ingreso a <@!${newMember.user.id}>: ${err.message} (${err.name})`
            )
            .catch(client.logException);
          client.logException(err);
        });
