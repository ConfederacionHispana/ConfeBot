    ...
    if (oldMember.pending && !newMember.pending) {
      ...
      if (minimumMS < ageMS) {
        ...
        if (!widgetInvite) client.logger.warn('No he podido encontrar una invitación por widget.');
          ...
          const usedWidgetInvite = widgetInvite.code === client.cache.widgetInvite.code
            && widgetInvite.uses === client.cache.widgetInvite.uses + 1;
          const usedNewWidgetInvite = widgetInvite.code !== client.cache.widgetInvite.code
            && widgetInvite.uses === 1;
          if (usedWidgetInvite || usedNewWidgetInvite) {
            ...
            newMember.kick('Cuenta creada hace menos de 3 días.').then(() => {
              // log everywhere
              client.logger.info(`<@!${newMember.user.id}> (${newMember.user.tag}) expulsado del servidor por tener menos de 3 días desde la creación de su cuenta.`);
              logsChannel.send(`<@!${newMember.user.id}> (${newMember.user.tag}) expulsado del servidor por tener menos de 3 días desde la creación de su cuenta.`).catch(client.logException);
      ...
      newMember.roles.add(env.NEWUSER_ROLE).then(() => {
        welcomeChannel.send(`¡Bienvenid@ <@!${newMember.user.id}> a la **Confederación de Fandom Hispano**!`, {
          ...
          sentMessage.awaitReactions(filter, {
            ...
            if (reaction && reaction.emoji.name === '🌐') {
              newMember.roles.add(env.USER_ROLE).then(() => {
                ...
                logsChannel.send(`✅ Se verificó a <@!${newMember.user.id}> como invitado`).catch(client.logException);
              }).catch((err) => {
          ...
          logsChannel.send(`❌ Imposible enviar mensaje de bienvenida a <@!${newMember.user.id}>: ${err.message} (${err.name})`).catch(client.logException);
          client.logException(err);
        });
      }).catch((err) => {
        logsChannel.send(`❌ Imposible agregar el rol Nuevo Ingreso a <@!${newMember.user.id}>: ${err.message} (${err.name})`).catch(client.logException);
        client.logException(err);
      });
    }
  }
