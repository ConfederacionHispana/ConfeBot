      ...
      if (differenceDays >= 2) {
        member
          ...
          .finally(() => {
            member
              .kick(
                'Expulsión automática pasadas 48 horas sin llenar el formulario'
              )
              .then(() => {
                client.logger.info(
                  `Kicked ${member.user.username}#${member.user.discriminator}`,
              ...
              .catch((err) => {
                logsChannel
                  .send(
                    `Error al expulsar a <@!${member.user.id}>: ${err.message} (${err.name})`
                  )
                  .catch(client.logException);
