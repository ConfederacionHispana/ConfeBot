...
class UserAssociateCommand extends Command {
  public async run(message: Message, args: Args): Promise<void> {
    ...
    if (!fandomUserMatch.success && !guestFlag) {
      message
        .reply(
          '❓ Se requiere un nombre de usuario de Fandom, o el flag `--guest` para verificar como invitado.'
        )
        .catch(client.logException);
      return;
    }
    ...
    if (fandomUser) {
      try {
        ...
        const { id } = targetUser;
        const dbUser =
          (await DBModels.User.findOne({ id })) || new DBModels.User({ id });
        ...
        message
          .reply(
            `❌ ${
              err instanceof NonExistentUser
    ...
    const logsChannel = message.guild?.channels.resolve(
      env.LOGS_CHANNEL
    ) as TextChannel;
    const rolesToAdd = fandomUser
      ? [env.USER_ROLE, env.FDUSER_ROLE]
      : [env.USER_ROLE];
    const logReason = `✅ <@!${message.member?.id}> verificó a <@!${
      targetUser.id
    }> ${
