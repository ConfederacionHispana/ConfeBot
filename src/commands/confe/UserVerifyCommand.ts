...
class UserVerifyCommand extends Command {
  public async run(message: Message, args: Args): Promise<void> {
    ...
    const fandomUserResolver = Args.make((arg) =>
      Args.ok(arg.substring(0, 255))
    );
    ...
    const interactiveVerifyURL = new URL(
      'https://confederacion-hispana.fandom.com/es/wiki/Especial:VerifyUser'
    );
    interactiveVerifyURL.searchParams.append('user', message.author.username);
    interactiveVerifyURL.searchParams.append(
      'tag',
    ...
    UserVerification.verifyUser(
      ...
      .then((result) => {
        if (result.success) {
          member.roles
            .add(
              [env.USER_ROLE, env.FDUSER_ROLE],
              `Verificado como ${fandomUser.value}`
            )
            .then(async () => {
              member.roles.remove(env.NEWUSER_ROLE).catch(client.logException);
              logsChannel
                .send(
                  `✅ Se verificó a <@!${message.author.id}> con la cuenta de Fandom **${fandomUser.value}**`
                )
          ...
          if (result.error === 'Blocked') {
            embed.description += `\n\nEl bloqueo fue efectuado por ${
              result.blockinfo?.performer
            }${
              result.blockinfo?.reason
                ? ` con la razón _${result.blockinfo.reason}_`
                : ''
            }, y expira ${
              result.blockinfo?.expiry === Infinity
                ? '**nunca**'
                : `el ${formatDate(
                    result.blockinfo?.expiry as Date,
                    'dd/MM/yyyy'
                  )}`
            }.`;
            logsChannel
          ...
          if (
            result.error === 'DiscordHandleNotFound' ||
            result.error === 'DiscordHandleMismatch'
          )
          ...
          if (result.error === 'DiscordHandleMismatch') {
            client.logger.info(
              'Usuario inició la verificación, discordHandle no coincide',
              {
                discordTag,
      ...
      .catch((err) => {
        if (err instanceof NonExistentUser) {
          client.logger.info(
            'Usuario inició la verificación, usuario de Fandom no existe',
            {
              discordTag,
              fandomUser: fandomUser.value
            }
